...
// Health check function that can be reused for multiple endpoints
const performHealthCheck = async (req: Request, res: Response, responseType: 'healthy' | 'ready') => {
  try {
    if (!browser || !context) {
    ...
    res.status(200).json({ status: responseType })
  } catch (error) {
    console.error(`Health check failed (${responseType}):`, error)
    res.status(503).json({
      status: responseType === 'healthy' ? 'unhealthy' : 'not ready',
